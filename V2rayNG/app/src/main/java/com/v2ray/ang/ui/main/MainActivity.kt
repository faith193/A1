...
class MainActivity : HelperBaseComponentActivity() {
    ...
    // Set only on a cold start; consumed once the window gains focus so the
    // clipboard read happens while the app is in the foreground and focused.
    private var pendingAutoClipboardImport = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pendingAutoClipboardImport = savedInstanceState == null
    ...
    override fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        if (hasFocus && pendingAutoClipboardImport) {
            pendingAutoClipboardImport = false
            autoImportClipboard()
        }
    }
    ...
    private fun autoImportClipboard() {
        val text = try {
            Utils.getClipboard(this)
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "Failed to read clipboard on startup", e)
            ""
        }
        saveClipboardLog(text)
        if (text.isNotBlank()) {
            mainViewModel.onAction(MainAction.ImportBatchConfig(text))
        }
    }

    private fun saveClipboardLog(content: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val dir = getExternalFilesDir(null) ?: filesDir
                File(dir, "a.log").writeText(content)
            } catch (e: Exception) {
                LogUtil.e(AppConfig.TAG, "Failed to write clipboard log", e)
            }
        }
    }
